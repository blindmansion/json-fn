...
these in the *shared* suite, not TS-only, since module scope makes the
collisions common):
...
- Lisp-2 asymmetry (module *constant* `map` vs module *function* `map`);
...
### `$literal` → `$raw`
Once the evaluator key is renamed (see parity item), add a `spec/cases` entry
pinning `{ "$raw": … }` as inert verbatim JSON (and, if kept, the `$literal`
alias). Guards against impls drifting on which key is honored.
