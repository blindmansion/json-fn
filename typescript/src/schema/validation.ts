...
export class SchemaFragmentValidationError extends Error {
  constructor(
    readonly code: "INVALID_SCHEMA_FRAGMENT",
    readonly path: string,
    message: string,
  ) {
    super(`${path}: ${message}`);
    this.name = "SchemaFragmentValidationError";
  }
}
...
function fail(path: string, message: string): never {
  throw new SchemaFragmentValidationError("INVALID_SCHEMA_FRAGMENT", path, message);
}
...
export function validateSchemaFragment(
  value: unknown,
  defs: Defs = {},
  path = "schema",
): asserts value is Schema {
...
export function validateDefinitionTable(value: unknown, path = "$defs"): asserts value is Defs {
  ...
  for (const [name, schema] of Object.entries(definitions)) {
    if (name.length === 0 || name.includes("/")) fail(`${path}.${name}`, "invalid definition name");
    validateSchema(schema, `${path}.${name}`, { defs, declaredTVars: null, usedTVars: new Set() });
  }
}

export function validateCallableSignature(
  value: unknown,
  defs: Defs = {},
  path = "signature",
): asserts value is CallableSignature {
