// Migrate the parse conformance suites (spec/parse-cases/*.json) from the legacy
// array-valued `$fn` call form to the split `$call`/`$args` form. The shorthand
// `source` is unchanged (its lowering target changed, not its syntax), so only
// each case's `expected` canonical JSON is rewritten via the shared
// `toSplitForm` utility.
//
...
// This requires the updated parser to already emit `$call`/`$args`.
//
...
import { toSplitForm } from "./fn-split-transform";
...
for (const file of readdirSync(DIR)
  ...
  for (const tc of suite.cases) {
    ...
    tc.expected = toSplitForm(tc.expected!);
    transformed++;
