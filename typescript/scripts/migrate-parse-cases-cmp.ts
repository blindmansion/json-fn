// Migrate the parse conformance suites (spec/parse-cases/*.json) from the
// first-class comparator nodes ($eq/$neq/$lt/$lte/$gt/$gte) and the unary $not
// node to stdlib function calls. The shorthand `source` is unchanged (its
// lowering target changed, not its syntax), so only each case's `expected`
// canonical JSON is rewritten via the shared `toCallForm` utility.
//
...
// This requires the updated parser to already lower `==`/`<`/`!`/… to calls.
//
...
import { toCallForm } from "./comparison-transform";
...
for (const file of readdirSync(DIR)
  ...
  for (const tc of suite.cases) {
    ...
    tc.expected = toCallForm(tc.expected!);
    transformed++;
